...
PS4.GetProcessList(GetProcessListCallback, FailCallback);
	...
	var tid = pi.titleid.trim();
		...
		gameSTAT.innerHTML="运行中的游戏ID: "+tid;
		setTimeout(SearchRunningGame,50);}
	});
});
}).fail(function(){sessionStorage.WebRTE="no";rteSTAT.innerHTML="WebRTE插件: <font color=#dc3545>未加载";});
...
GetRunningCUSA();
setTimeout(function(){if(sessionStorage.runningCUSA==null){gameSTAT.innerHTML="运行中的游戏ID: <font color=#dc3545>未运行";}},2000);
});
